---
title: "Scraping bibliometric data using R"
author: "C?sar Villamizar"
date: "April 26, 2019"
...
```{r, a, message=FALSE, warning=FALSE, paged.print=FALSE}
library(rvest)
library(xml2)
library(httr)
library(tidyverse)
...
Using the information obtained from the Wikipedia page, I will create a data frame with a unique identifier by institutions, removing words like University and Universit?.

```{r, e, include=TRUE, eval=FALSE }
# Creating a normalized list of u15 universities
u15 <- u15 %>% 
  stringr::str_replace("University of", " ") %>% 
  stringr::str_replace("University", " ") %>% 
  stringr::str_replace("Universit? de", " ") %>% 
  stringr::str_replace("Universit?", " ") %>% 
...
```{r include=TRUE, eval=FALSE}
u15_html <- readRDS ("./data/u15_html.RDS") 
